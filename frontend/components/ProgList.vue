  ...
  <v-card class="cardsep" v-for="prog in progs" :key="prog.id" :to="`/prog/${prog.id}`">
    <v-img height="300" cover :src="`https://tv.glynnbird.com/api/img?id=${prog.id}`">
      ...
      <v-card-subtitle class="position-absolute bottom-0 left-0 offbot">
        <v-chip class="sep" v-if="prog.on" variant="flat" color="grey-lighten-2" label size="small">{{ prog.on }}</v-chip>
        <v-chip class="sep" variant="flat" label size="small" color="grey-lighten-2" v-if="prog.uptoep && prog.uptomax">{{ prog.uptoep }} / {{ prog.uptomax }}</v-chip>
        <v-chip class="sep" variant="flat" label size="small" color="grey-lighten-2" v-if="prog.type==='Film' || prog.type=='Single'">{{ prog.type }}</v-chip>
