...
* // returns ''
*/
export const porterStemmer = function ( word ) {
	var firstch;
	var suffix;
	var stem;
	var fp;
	...
	if ( !isString( word ) ) {
		throw new TypeError( `invalid argument. First argument must be a string. Value: ${word}.`);
	}
	if ( word.length < 3 ) {
		return word;
	}
	...
	if ( word.endsWith('ll') && RE_MGR1.test( word ) ) {
		word = word.replace(RE_LAST, '' );
	}
